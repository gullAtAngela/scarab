...
function list_render($label, $options, $name, &$request = null) {
	...
	$output = "<label class=\"form-label\" for=\"$name\">$label</label>\n";
...
function  inputText($label, $name, $value, &$request = null) {
	$request === NULL 
		...
		$wert = $request[$name];
		$output = "<label class=\"form-label\" for=\"$name\">$label</label>";
		$output .= "<input class=\"form-control\" type=\"text\" name=\"$name\" value=\"$wert\"><br />\n";
		return $output;
}

function input($type, $name, $label) {
	$output = '<label class="form-label" for="' . $name . '">' . $label . '</label>';
	$output .= '<input class="form-control" type="' . $type . '">';
